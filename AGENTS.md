...
- Version 0.16.1 validates every MCP tool argument object against the published `ToolSpec.inputSchema` before local or bridge dispatch using a dependency-free JSON Schema subset. Explicit falsy non-object `params` and `arguments` are invalid; retain editor-side validation as defense in depth. Release checks must continue to derive consistency only from executable, package, plugin, and runtime sources, never documentation contents.
- The 0.16.1 localhost bridge retains at most 16 clients. A newly accepted client has two seconds to authenticate and submit one complete request; excess, idle, and incomplete clients are disconnected. Debugger-deferred clients occupy the same cap and remain additionally bounded by the runtime pending-request limit and timeout. Token read, generation, write, or flush failure must abort both listener and discovery startup.
- Staged imports publish the completed same-directory temporary file atomically without replacement. A destination created at any time before publication must remain unchanged and return the stable already-exists domain error on both POSIX and Windows branches.
- Runtime probe protocol 2 is unchanged in 0.16.1. `runtime_probe.gd` is only the debugger-autoload composition/protocol shell; runtime identity/path confinement, tree/property inspection, capture staging, injected-input scheduling, and condition evaluation belong to their focused runtime service modules. Preserve the absence of game-side networking, arbitrary mutation, method execution, expressions, and supplied-code evaluation across every service.
