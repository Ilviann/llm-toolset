...
Read [`docs/workflow.md`](docs/workflow.md) before making workspace or application changes.
...
Store feature documents under `docs/features/<status>/`, where status is `planned`, `active`, `completed`, or `deferred`. Begin each feature document with YAML front matter containing string `feature_id`, enum `status`, string-list `depends_on`, and nullable string `released_in`; the status must match its directory, release metadata must follow [`docs/workflow.md`](docs/workflow.md), and front-matter dependencies must match the document's direct-prerequisite section. Dependencies may name stable feature IDs or explicit issue IDs.
...
- Keep each roadmap feature usable and releasable. Maintain a checkbox feature checklist; classify and version completed work according to [`docs/workflow.md`](docs/workflow.md), and synchronize all required version sources and history.
