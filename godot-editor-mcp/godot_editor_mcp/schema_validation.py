"""Dependency-free validation for the JSON Schema subset in the tool catalog."""
...
class SchemaValidationError(ValueError):
    """A model-facing tool argument does not match its published schema."""


def validate_tool_arguments(value: Any, schema: Mapping[str, Any]) -> None:
    """Validate *value* against the bounded vocabulary used by ToolSpec schemas."""
...
def _validate_object(
    ...
    for name, item in value.items():
        ...
        if isinstance(child_schema, Mapping):
            _validate(item, child_schema, root, f"{path}.{name}")
