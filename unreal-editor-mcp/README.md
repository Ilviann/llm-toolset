...
Read-only inspection includes Function/Macro Libraries, Interface declarations and authored graph structure, effective inherited component values with origins, exact reflected property selection, and typed Gameplay Attributes without a base GAS dependency. See [inspection selectors](docs/user/asset-inspection.md#expanded-blueprint-and-reflected-selectors).
...
The optional `UnrealMCPGAS` 0.4.0 companion adds bounded read-only `asset_inspect` blocks and selectors for Gameplay Ability, data-only Gameplay Effect, Cue Notify, Attribute Set, magnitude-calculation, and execution-calculation Blueprints without adding GAS dependencies to the base plugin. The Windows graphical deployment helper can build and install it alongside the base plugin. See the [Gameplay Ability](docs/user/gameplay-ability-blueprints.md), [Gameplay Effect](docs/user/gameplay-effects.md), and [supporting GAS asset](docs/user/gas-supporting-assets.md) guides.
...
Select the folder containing the game's `.uproject` and the matching Unreal Engine 5.8+ installation. Optionally select the independent **Unreal MCP GAS companion plugin**, **Unreal MCP CommonUI companion plugin**, **Unreal MCP Enhanced Input companion plugin**, and **Unreal MCP AI companion plugin** checkboxes, then choose project installation with explicit `.uproject` enablement, Engine installation enabled by default, or Engine installation without default enablement. Engine plugins are installed under `<Engine>\Engine\Plugins\Marketplace`; project plugins are installed under `<YourProject>\Plugins`. Binary deployment strips module-owned and UAT-generated implementation source while retaining every required precompiled module rule. Existing selected-plugin installations require approval before replacement. Matching Win64 PDB deployment, writable MCP tools, and editor lifecycle control remain independent options. The window has **Build log output** and **MCP settings preview** tabs; after installation, the settings preview contains sample entries for LM Studio `mcp.json` and ChatGPT Codex `config.toml`.
...
- Semantic asset and game-data inspection: `asset_inspect` deeply reads supported gameplay/Data Asset/Widget/Animation Blueprints, Data Asset instances, Data Tables, and optional Enhanced Input/GAS/CommonUI families; `blueprint_action_catalog` and `game_data_inspect` retain their focused contracts.
- Exact Gameplay Tag properties: existing reflected-value workflows read and set `FGameplayTag` strings and deterministic explicit `FGameplayTagContainer` arrays without requiring the GAS companion.
