...
Unreal Editor MCP 0.7.0 is an offline-first MCP bridge for Unreal Engine 5.8+. It pairs a dependency-free Python 3.10+ stdio server with an editor-only C++ plugin. This release exposes exactly ten tools:
...
Phase 7 supports reliable Actor Blueprint creation, component hierarchy/default editing, typed variables, user-function shells, locals, macros, custom-event shells, RepNotify coupling, compilation, and saving. General graph-node mutation, editor lifecycle, build, filesystem, console, unrestricted reflection, and code execution remain unavailable.
...
3. Open the project. Look for `Unreal MCP 0.7.0 ready on 127.0.0.1:15485` in the editor log.
...
The 0.7.0 native baseline is Unreal 5.8.0 on Apple Silicon macOS 26.5.2 with Xcode 26.1.1. Windows and Linux path/process branches are unit-tested through injected adapters; native Windows qualification remains a later roadmap gate.
