...
Unreal Editor MCP 0.51.0 is an offline-first MCP bridge for Unreal Engine 5.8+. It pairs a dependency-free Python 3.10+ stdio server with an editor-only C++ plugin.
...
3. Open the project and wait for `Unreal MCP 0.51.0 ready on 127.0.0.1:15485` in the editor log.
...
- Semantic asset and game-data inspection: `asset_inspect` deeply reads supported gameplay/Data Asset/Widget/Animation Blueprints, Data Asset instances, and Data Tables; `blueprint_action_catalog` and `game_data_inspect` retain their focused contracts.
