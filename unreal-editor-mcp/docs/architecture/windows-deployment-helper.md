...
`tests/test_deploy_plugin_windows.py` covers bounded project parsing, exact Engine-association candidate selection, 5.8+ validation, fixed Win64 arguments, default symbol filtering, opt-in matching-PDB retention and enforcement, guarded precompiled-module-rule configuration, precompiled-metadata retention, explicit replacement, non-mixing replacement, failed-verification restoration, result verification, exact readonly/writable/lifecycle LM Studio JSON, and fail-closed lifecycle-executable validation. `tests/test_package_plugin.py` continues to cover the shared packaging boundary. A native plugin build is required when the source plugin or its packaging compatibility changes; the helper itself has no Unreal runtime behavior.
