...
- One editor transaction owns each admitted edit. A semantic rejection or no-op whose snapshot stayed unchanged cancels without disturbing prior transactions and restores the package's admitted dirty state. Successful edits optionally persist and must return an exact postcondition snapshot; changed-snapshot verification is explicit.
