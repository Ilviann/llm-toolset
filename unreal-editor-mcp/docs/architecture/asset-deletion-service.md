...
`FUnrealMCPAssetDeletionService` owns exact single-asset and complete map-owned-package deletion preflight, mutation-scope enforcement, public Unreal deletion calls, and registry/storage verification. It depends on `FUnrealMCPAssetReferenceService` for a fresh full snapshot without creating a cursor. `UnrealMCPBridge` owns operation admission and rejects concurrent retained mutations.
...
- Ordinary assets remain single-package. A map is accepted only through the map branch, where current/streaming/dirty/read-only targets reject and the complete bounded root/build-data/external package closure is reference-checked and verified.
