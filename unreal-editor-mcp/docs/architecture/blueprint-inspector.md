...
- An admitted read-only companion Blueprint family follows the same discovery, base collectors, paging, and non-mutation checks. Its bounded handler records and full fingerprint join the base sink so one snapshot covers ordinary and companion-owned state. `UnrealMCPGAS` uses this seam for `gameplay_ability`.
...
- UE 5.8's hidden, untyped, unlinked, default-free `ErrorTolerance` input on `K2Node_PromotableOperator` is editor-derived rather than structural: it is omitted from pin records and fingerprints because Unreal regenerates only its GUID on first reload. Any typed, visible, linked, or default-bearing tolerance pin remains structural.
