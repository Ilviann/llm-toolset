...
- Default inspection is shallow: summary, parent, compile state, components, typed variables, user functions/signatures, locals, and graphs. Nodes, pins, connections, and separate parameter records require explicit sections. `member_id`, `function_id`, and `local_id` select one stable variable, function graph, or function-local variable.
...
- Function records distinguish user-owned, inherited, override, and interface functions; expose complete access/pure/const signatures, metadata, required entry/result nodes, RepNotify members, and bounded references. Parameter records preserve order/direction/reference/const/default forms. Local records carry stable GUIDs, exact function scope, type/default, editability, and scope-aware references.

## Verification

`UnrealMCP.Phase2` covers discovery, structure, paging, identity, and non-mutation. `UnrealMCP.Phase4` adds component/default read-back. `UnrealMCP.Phase5` adds exact variable inspection. `UnrealMCP.Phase6` adds exact function/signature/parameter/local/RepNotify inspection before and after compile/save/restart. The cross-process script compares persisted snapshots through the production Python client.
