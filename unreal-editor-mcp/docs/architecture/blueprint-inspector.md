...
- Default inspection is shallow: summary, parent, compile state, components, typed variables, and graphs. Nodes, pins, and connections require explicit sections. `member_id` selects one stable variable.
...
- Variable records expose canonical K2 types/tagged defaults, metadata, ownership/editability, replication and immutable RepNotify relationships, plus at most 64 exact loaded graph/node references and an unresolved-reference signal.

## Verification

`UnrealMCP.Phase2` covers discovery, structure, paging, identity, and non-mutation. `UnrealMCP.Phase4` adds component/default read-back. `UnrealMCP.Phase5` adds exact variable type/default/metadata/replication/reference inspection before and after compile/save/restart. The cross-process script compares persisted snapshots through the production Python client.
