...
- `blueprint_block_replace` has an exact legacy complete-function shape plus exact uniform function, macro, custom-event, and native-event shapes with snapshot, boundary, external-link, action, position, default, connection, conversion, and logic-unit-fingerprint preconditions.
