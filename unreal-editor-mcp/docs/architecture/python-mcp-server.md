...
`unreal_editor_mcp/` owns the Python 3.10+ process. `stdio.py` bounds newline-delimited JSON-RPC and keeps stdout protocol-only. `server.py` negotiates MCP, consumes one access- and capability-filtered catalog composition, validates arguments, composes local and native capability fields, and converts domain failures to MCP tool errors. `asset_family_catalog.py` owns the immutable schema, access, command, result, native-requirement, and companion publication descriptors over the focused definitions in `tool_catalog_families/`; `tool_catalog.py` retains compatibility exports. `project.py`, `platforms.py`, and `discovery.py` resolve one project, derive its non-path identity, and validate generated state. `bridge.py` is the only HTTP client. `lifecycle.py` owns configured editor process orchestration and durable lifecycle records. `cli.py` composes these responsibilities.
...
- Readonly is the default. Its exact ordered catalog is `capabilities`, `editor_state`, `operation_status`, `asset_inspect`, `asset_references`, `level_inspect`, `level_open`, `blueprint_action_catalog`, and `game_data_inspect`.
...
- The public catalog order is assembled from validated immutable family descriptors; every family, tool, command mapping, companion, and contribution identity is unique. Access, lifecycle, and authoritative native-command requirements filter exact entries without mutating schemas. A call to an omitted tool fails as unknown before bridge dispatch or argument validation.
...
- `asset_inspect` requires one exact `/Game` asset and accepts optional selector, verbose, page-size/page-index, and partial-graph fields. Python validates exact shapes and bounds before HTTP; successful native JSON is rendered as deterministic safe YAML only at the MCP text boundary.
- `stdio.py` reconfigures stdin, stdout, and stderr as strict UTF-8 before serving so valid Unicode cannot terminate the server under a non-UTF-8 inherited Windows locale.
...
- `blueprint_block_replace` has an exact legacy complete-function shape plus exact uniform explicit-position and `layered_v1` function, macro, custom-event, and native-event shapes with snapshot, boundary, external-link, action, placement, default, connection, conversion, and logic-unit-fingerprint preconditions.
