...
`UnrealMCP.AssetInspect.CoreFamiliesSelectorsPagingAndLimits` covers canonical paths, every core classification including Data Asset Blueprints, neutral/media behavior, selectors, UTF-8 encoding, paging, Interface declarations, snapshots, invalid/security cases, non-mutation, and complete/partial graph limits through registry-backed dispatch. `UnrealMCP.AssetInspect.DataAssetsTablesSelectorsAndSnapshots` covers Data Asset/Data Table descriptors, structured values, nested pages, columns, snapshots, and non-mutation. Companion Automation covers overlay admission, route collisions, exact blocks, cumulative snapshots, repeatability, and unchanged state. Python tests cover exact family-set policy, schemas, deterministic YAML, strict types/escaping, and UTF-8 stdio. Lifecycle acceptance exercises built-in and companion root/selector reads through MCP YAML framing across restart.
