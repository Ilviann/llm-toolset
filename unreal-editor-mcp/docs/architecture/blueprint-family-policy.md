...
- Capabilities publish a bounded eleven-record base family/operation matrix. Discovery records and all exact-asset operation results report `blueprint_family`; exact inspection also reports live default, component, widget-tree, event-graph, local-variable, override, and graph-type capabilities.
- Animation Blueprints classify as inspection-only `animation`; unloaded discovery resolves the native AnimInstance parent tag, and exact inspection verifies `UAnimBlueprint`. Class-based creation remains excluded. The family enables only discovery and inspection and publishes no RPC modes. Live graph capabilities add animation, state-machine, state, and transition inspection.
...
- `BPTYPE_Interface` classifies as inspection-only `interface` from the registry tag or loaded asset enum. It publishes only discovery and inspection, with function graph support and no defaults, components, locals, overrides, or RPC modes. Class-based authoring remains excluded. Animation Layer Interfaces retain the specialized `animation` family; unloaded discovery recognizes their native `UAnimLayerInterface` parent.
...
`UnrealMCP.Interfaces.InterfaceInspection` verifies interface classification, signatures, ownership, graph selection, paging, stale cursors, and mutation rejection. `--interface-only` validates unloaded discovery and unchanged saved assets across editor restarts.
