...
`plugin/UnrealMCP/Source/UnrealMCP/` is one editor-only module. `UnrealMCPModule.cpp` is the composition root and owns configured-port validation, stale-record cleanup, token startup gating, project hashing, loopback listener configuration, and bridge lifetime. `UnrealMCPTokenStore` owns credential persistence. `UnrealMCPBridge` owns the HTTP route, authentication, request queue, Game-thread dispatch, command composition, heartbeat, and process identity. `UnrealMCPOperationLedger` owns mutation admission and retained outcomes. `UnrealMCPBlueprintFamilyPolicy` owns classification and multiplayer capability publication. `FUnrealMCPGameplayFrameworkEditor` owns narrow project-default assignment. `FUnrealMCPGameDataService` owns user-defined structs and Data Tables. `FUnrealMCPLevelService` owns map discovery, identities, revisions, snapshots, cursors, and safe opening. `FUnrealMCPLevelManagementService` owns exact map creation, bounded World Settings setup, saving, and reload verification. `FUnrealMCPAssetReferenceService` owns exact mounted targets, reference evidence, reference snapshots, and cursors. `FUnrealMCPAssetDeletionService` owns ordinary single-package and bounded map-package-closure deletion preflight and persistence verification.
...
- The route accepts POST at `/unreal-mcp/v1/command`; the command allowlist contains the twenty-two model-facing commands released through `level-management` plus internal `editor_shutdown`.
