...
`plugin/UnrealMCP/Source/UnrealMCP/` is one editor-only module. `UnrealMCPModule.cpp` is the composition root and owns configured-port validation, stale-record cleanup, token startup gating, project hashing, loopback listener configuration, and bridge lifetime. `UnrealMCPTokenStore` owns credential persistence. `UnrealMCPBridge` owns the HTTP route, authentication, request queue, Game-thread dispatch, command composition, heartbeat, and process identity. `UnrealMCPOperationLedger` owns mutation admission and retained outcomes. `UnrealMCPBlueprintFamilyPolicy` owns explicit Actor/GameMode/GameState classification and capability publication. Inspector, mutator, action-catalog, and graph-editor components consume that shared policy before live Blueprint access.
...
- The route accepts POST at `/unreal-mcp/v1/command`; the command allowlist contains the twelve commands released through Phase 14.
