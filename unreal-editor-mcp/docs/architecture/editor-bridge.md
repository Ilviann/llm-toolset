...
`plugin/UnrealMCP/Source/UnrealMCP/` is one editor-only module. `UnrealMCPModule.cpp` is the composition root and owns configured-port validation, stale-record cleanup, token startup gating, project hashing, loopback listener configuration, and bridge lifetime. `UnrealMCPTokenStore` owns credential persistence. `UnrealMCPBridge` owns the HTTP route, authentication, request queue, Game-thread dispatch, command composition, heartbeat, and process identity. `UnrealMCPWireTypes` owns JSON-neutral request, result, error, value, capability, identity, selector, paging, diagnostic, mutation, and persistence records; `UnrealMCPJsonCodec` owns their explicit bounded JSON conversion. `UnrealMCPOperationLedger` owns mutation admission and retained outcomes. `UnrealMCPBlueprintFamilyPolicy` owns classification and multiplayer capability publication. `FUnrealMCPGameplayFrameworkEditor` owns narrow project-default assignment. `FUnrealMCPGameDataService` owns user-defined structs and Data Tables. `FUnrealMCPLevelService` owns map discovery, identities, revisions, snapshots, cursors, and safe opening. `FUnrealMCPLevelManagementService` owns exact map creation, bounded World Settings setup, saving, and reload verification. `FUnrealMCPAssetReferenceService` owns exact mounted targets, reference evidence, reference snapshots, and cursors. `FUnrealMCPAssetDeletionService` owns ordinary single-package and bounded map-package-closure deletion preflight and persistence verification.

## Dependency direction

The module constructs the token store and bridge. The protocol decodes HTTP JSON into neutral records before queueing, base domain services consume and return only neutral records, and the protocol encodes the final envelope after dispatch. The companion registry is the sole temporary base-domain adapter to companion API v1 JSON. The bridge calls the protocol and discovery helpers but none of those helpers knows about module lifetime. Unreal object/editor state is read only after `AsyncTask` reaches the Game thread. The HTTP handler performs authentication, body bounds, shape validation, command allowlisting, and queue admission before dispatch.
