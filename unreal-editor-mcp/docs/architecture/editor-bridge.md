...
`plugin/UnrealMCP/Source/UnrealMCP/` is one editor-only module. `UnrealMCPModule.cpp` is the composition root and owns configured-port validation, stale-record cleanup, token startup gating, project hashing, loopback listener configuration, and bridge lifetime. `UnrealMCPTokenStore` owns credential persistence. `UnrealMCPBridge` owns the HTTP route, authentication, request queue, Game-thread dispatch, command composition, heartbeat, and process identity. `UnrealMCPOperationLedger` owns mutation admission and retained outcomes. `UnrealMCPBlueprintFamilyPolicy` owns classification and multiplayer capability publication. `FUnrealMCPGameplayFrameworkEditor` owns narrow project-default assignment.
...
- The route accepts POST at `/unreal-mcp/v1/command`; the command allowlist contains the thirteen commands released through Phase 16.
