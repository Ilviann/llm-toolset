...
`Private/Tests/UnrealMCPAutomationTestSupport.h` owns shared fixture construction, argument builders, inspection helpers, snapshot tracking, save support, and cleanup conventions. Phase-specific translation units contain independently registered cases under their `UnrealMCP.PhaseN` filters. Seven Phase 1 cases cover the authenticated foundation; three Phase 2 inspection; three Phase 3 creation; three Phase 4 ledger/property/component-default; two Phase 5 K2/member; and the focused Phase 6, 7, 8, 10, and 11 scenarios cover functions/locals/RepNotify, macros/custom events, core/expanded action catalogs, and graph-node lifecycle. `UnrealMCPApiProbe.cpp` keeps public-header compatibility a normal compilation requirement. Run all 23 cases through `scripts/run_headless_integration.py --automation-only`.
...
`scripts/run_headless_integration.py` uses only the disposable `ue-test/` project. It calls every released command family through the production Python client, reconciles deliberately lost component and graph add/move/remove responses, and builds/saves a multi-component Actor Blueprint with defaults, variables, functions, locals, RepNotify, a macro, a custom event, and a positioned graph node. It then restarts the editor and verifies stable identities, exact signatures/defaults/metadata/relationships, required macro tunnels, custom-event graph placement, surviving node/pin identities and position, removed-node absence, the exact snapshot, every released action family, wildcard operators, representative serialized catalog size, loopback binding, and clean heartbeat removal.
