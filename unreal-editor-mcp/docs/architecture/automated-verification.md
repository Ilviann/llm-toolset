...
`scripts/run_headless_integration.py` uses only the disposable `ue-test/` project. It calls every released command family through the production Python client, reconciles a deliberately lost mutation response, and builds/saves a multi-component Actor Blueprint with defaults, variables, functions, locals, RepNotify, a macro, and a custom event. It then restarts the editor and verifies stable identities, exact signatures/defaults/metadata/relationships, required macro tunnels, custom-event graph placement, the exact snapshot, and local function/variable catalog results before checking loopback binding and clean heartbeat removal.
