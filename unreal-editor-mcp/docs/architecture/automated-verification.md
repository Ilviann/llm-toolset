...
`Private/Tests/UnrealMCPAutomationTests.cpp` compiles into development Editor builds. Seven `UnrealMCP.Phase1` cases cover the authenticated foundation. Three `UnrealMCP.Phase2` cases cover inspection. Three `UnrealMCP.Phase3` cases cover creation policy, compile/save behavior, cleanup, diagnostics, local-plugin confinement, and a persisted creation fixture. Three `UnrealMCP.Phase4` cases cover the operation ledger, shared property codec, and component/default edits. Two `UnrealMCP.Phase5` cases cover the K2 type/default codec and member-variable lifecycle. `UnrealMCP.Phase6.FunctionsAndLocals` covers complete signatures, metadata, required nodes, stable local identities, references, RepNotify coupling, Undo/Redo, compile, and save. `UnrealMCPApiProbe.cpp` makes public-header compatibility a normal compilation requirement. Run all 19 cases through `scripts/run_headless_integration.py --automation-only`; its captured output avoids macOS GUI-launch behavior that can make direct pipe-based runners unreliable.

## Cross-process boundary

`scripts/run_headless_integration.py` uses only the disposable `ue-test/` project. It prepares the inspection fixture, starts an editor, calls every released command family through the production Python client, deliberately discards a component-edit response, reconciles and replays the retained result, builds and saves a multi-component Actor Blueprint with edited class/member defaults, function signatures/metadata, locals, and RepNotify coupling, rejects a wrong token, and proves loopback-only listening. It then restarts the editor and verifies parent, compile state, hierarchy, class default, stable member/function/local identities, exact signatures/defaults/metadata/replication relationships, and exact snapshot before checking clean heartbeat removal.
