...
- Before invocation, the retained signature is scanned from the live action database and passed through the same live graph and optional pin filter. A missing, expired, mismatched, filtered, or unresolvable identity returns `invalid_action` before a transaction.
