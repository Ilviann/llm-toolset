# Unreal Editor MCP user guides

Start with the [project README](../../README.md) for installation, first connection, and the concise contract overview. Use the [tool guide overview](tool-guides.md) for the released tool inventory.

## Detailed guides

- [Setup and operation](setup-and-operation.md) — security, deployment, LM Studio, and optional editor lifecycle.
- [Levels and assets](levels-and-assets.md) — level discovery, inspection, opening and management, asset references, and safe deletion.
- [Blueprint inspection](blueprint-inspection.md) — families, snapshots, identities, pagination, and mutation scope.
- [Blueprint graph authoring](graph-authoring.md) — action discovery, node lifecycle, function replacement, pin defaults, and connections.
- [Blueprint mutation](blueprint-mutation.md) — reliable mutations, creation, components, defaults, members, functions, locals, macros, and custom events.
- [Gameplay frameworks and data](gameplay-and-data.md) — GameMode, GameState, GameInstance, multiplayer assignment, structs, and Data Tables.
- [Widget Blueprint authoring](widget-blueprints.md) — hierarchy, layout, styling, property bindings, Designer events, safety, and limits.
- [Limits and offline testing](limits-and-testing.md) — published bounds and development validation commands.
- [Complete configurations and request sequences](../../examples/).

Executable schemas and the runtime `capabilities` response remain authoritative when a guide and a running version differ.
