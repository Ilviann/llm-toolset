...
See [`../../examples/widget-tree-workflow.json`](../../examples/widget-tree-workflow.json) for basic hierarchy creation. `asset_inspect` now composes ordinary Blueprint members and graph selectors with the base UMG semantic view: class defaults, effective inherited/local hierarchy, per-widget presentation, panel-slot layout, named slots, legacy property bindings, and Designer event bindings. Use `widget_tree`, `widgets/<name>`, `named_slots`, `bindings`, and `properties` with zero-based paging; collection defaults continue through exact nested selectors. Widget Animation timelines, CommonUI-specific records, MVVM records, and runtime widget state remain excluded from the base view.
