...
Unreal Editor MCP 0.53.0 is an offline-first MCP bridge for Unreal Engine 5.8+. It pairs a dependency-free Python 3.10+ stdio server with an editor-only C++ plugin. Readonly mode is the default and exposes these nine tools in deterministic order:
...
- `asset_inspect` returns deterministic safe YAML for one exact `/Game` asset, with compact semantic roots and hierarchical selectors for supported core Blueprint, base UMG Widget Blueprint, Animation Blueprint, Data Asset, Data Table, and optional companion families including Enhanced Input and AI assets.
