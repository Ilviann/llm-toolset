...
After installation, open the project and wait for the Unreal MCP ready message in the editor log. The **LM Studio JSON** tab provides **Copy JSON** for the complete `mcpServers` object to merge into LM Studio's `mcp.json`. The **ChatGPT Codex STDIO** tab provides separately copyable Name, Command/Python, and ordered Argument fields for **Settings → MCP servers → Add server**. The shared definition uses the Python interpreter that launched the helper, this checkout's `server.py`, and the selected absolute `.uproject` path. Depending on the independent choices, it also contains `--writable` and/or `--editor-lifecycle <selected-Engine-UnrealEditor.exe>`; it never contains the bridge token. Keep this checkout, Python executable, and any configured editor executable at those paths while either host uses the entry.
...
3. Open the project. Look for `Unreal MCP 0.44.0 ready on 127.0.0.1:15485` in the editor log.
