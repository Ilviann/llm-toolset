...
Select the folder that directly contains your game's `.uproject` file. When `UE58` is set, its value is already shown in the Unreal Engine field and is preserved after project selection when valid. Otherwise, the helper detects a matching Unreal Engine 5.8+ installation from `EngineAssociation` and standard Epic/user-build registry records; use the second Browse button if manual selection is needed. Select the GAS, CommonUI, Enhanced Input, and/or AI companion checkboxes when those optional plugins are required. For readable plugin frames in Windows crash reports, enable **Include matching PDB crash symbols (larger installation)**. The generated MCP entries are readonly by default. Enable **writable MCP tools** only for a dedicated trusted project, and enable **editor lifecycle control** independently to add the selected Engine's validated `UnrealEditor.exe`. Click **Build and install selected plugins**.

The helper uses the installed Engine and Visual Studio toolchain to package `Win64`. It can also package `UnrealMCPGAS`, `UnrealMCPCommonUI`, `UnrealMCPEnhancedInput`, and `UnrealMCPAI` with the base dependency when their independent companion checkboxes are selected. Choose project installation to deploy under `<YourProject>\Plugins` and enable the selected plugins in the `.uproject`, or choose either Engine mode to deploy under `<Engine>\Engine\Plugins\Marketplace` with `EnabledByDefault` set to the selected state. All selected packages build and verify before installation; replacement and project enablement roll back as one operation on failure.

Implementation source and external debug/symbol artifacts are removed by default. When PDB deployment is enabled, only PDBs directly under `Binaries/Win64` whose basenames match deployed DLLs are retained, and a missing match rejects installation. Every module declared by the installed descriptor retains its small module rule and precompiled Unreal Build Tool metadata, with `bUsePrecompiled = true` added so later game-project builds consume every packaged module. Existing selected-plugin folders require confirmation before replacement.
...
3. Open the project. Look for `Unreal MCP 0.53.1 ready on 127.0.0.1:15485` in the editor log.
