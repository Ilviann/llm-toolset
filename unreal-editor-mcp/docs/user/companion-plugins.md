...
Package the released `UnrealMCPGAS` 0.4.0 Gameplay Ability, Gameplay Effect, Cue Notify, Attribute Set, and calculation inspection companion with `--gas-companion`; it requires an API-v2-compatible base package. Package the released `UnrealMCPCommonUI` 0.3.0 Widget Blueprint inspection companion with `--commonui-companion`. On Windows, the graphical deployment helper provides independent GAS and CommonUI companion checkboxes and installs every selected plugin with the compatible base package in one transaction.
