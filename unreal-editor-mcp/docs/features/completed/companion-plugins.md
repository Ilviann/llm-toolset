...
- Add one narrow public native extension API and a base-owned registry. An enabled editor companion declares a required `UnrealMCP` plugin dependency without a semantic-version pin, a stable lowercase ASCII extension ID using only alphanumerics, `-`, and `_`, one supported extension-schema revision, its owning module with `LoadingPhase: None`, its independent companion semantic version, and one explicit positive integer `companion_api_version` whose initial value is `1`; the base registry explicitly loads the module and it registers through that API during startup.
