...
- Add the single `editor_lifecycle` tool with a typed `launch` operation. The later [`readonly-mode`](readonly-mode.md) configuration makes its validated absolute executable an independent opt-in; keep normal state reporting in the existing editor-state surface.
...
- Feature completion requires the implemented, documented, and automated launch contract. Missing applicable native platform verification is tracked separately in the roadmap platform test backlog.
