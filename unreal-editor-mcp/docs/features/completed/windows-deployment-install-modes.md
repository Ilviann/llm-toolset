...
**Implementation status:** Completed in 0.32.0 on unchanged companion API v1 and extension-schema revision 1. The independently versioned GAS companion advanced to 0.2.1 with the 0.32.1 startup-order fix.
...
- Python deployment tests cover fixed base/GAS build commands, source-descriptor restoration, stripped-metadata rejection, all destinations and enablement states, bounded descriptor mutation, invalid inputs, per-plugin binary filtering, pair installation, and restoration after a final enablement failure.
