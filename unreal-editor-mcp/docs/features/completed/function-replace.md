...
**Implementation status:** Completed in 0.23.0 and verified natively on macOS and Windows.
...
- Require explicit positions for changed nodes. Automatic layout remains unsupported until `node-layout`.
...
- Complete the feature only when unrelated-content fingerprints remain stable across successful, rejected, timed-out, and replayed function replacements on both native platforms.
