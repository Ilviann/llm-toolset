...
- Inspect and mutate a small explicit allowlist of Gameplay Ability configuration proven against Unreal Engine 5.7 public APIs, including supported activation/instancing/network policies, Gameplay Tag and trigger containers, and compatible cost/cooldown Gameplay Effect class references. Give nested records stable identities where names or indexes are insufficient and require the authoritative asset snapshot for every mutation.
