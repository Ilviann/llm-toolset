...
# `pie-lifecycle` — Retained operations and single-process multiplayer PIE lifecycle
...
- [`level-open`](../completed/level-open.md)
...
- Advertise only `single_process` until `pie-multiprocess` completes; reject `multi_process` with a stable capability error.
...
- Complete the feature only when repeated two-player listen-server start/stop is idempotent, bounded, fully observable, and returns the editor to ready with no leaked PIE worlds.
