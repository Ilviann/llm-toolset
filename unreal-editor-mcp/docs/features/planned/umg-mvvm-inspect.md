...
- Reuse `blueprint_inspect` for bounded asset summary, defaults, members, graphs, nodes, pins, diagnostics, references, and one authoritative asset snapshot. Publish an exact non-Actor family capability matrix and reject Actor components, Actor replication settings, creation, default/member/graph mutation, action cataloging, compilation, saving, and unrelated specialized-family operations.
