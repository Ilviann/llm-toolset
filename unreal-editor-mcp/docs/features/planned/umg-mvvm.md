...
- Reuse `blueprint_create`, `blueprint_member_edit`, `blueprint_action_catalog`, `blueprint_graph_edit`, `blueprint_compile`, and `blueprint_save`; use the prerequisite `blueprint_inspect` extension for stale-safe snapshots and read-back. Preserve the exact non-Actor family capability matrix and reject Actor components, Actor replication settings, and unrelated specialized-family operations.
