...
- Extend release-contract tests to compare Python, native, base descriptor, and companion descriptor versions and the companion's requested base version. Publish companion presence, version, readiness, and separate GAS inspection and mutation capabilities through `capabilities`. Fail closed and expose no GAS family when the companion, base, Python server, Unreal build, or Engine GAS dependency is missing or mismatched.
...
- Document companion installation and enablement, exact lockstep versioning, read-versus-mutation capability detection, supported Gameplay Ability inspection sections, inheritance, pagination, limits, exclusions, and focused inspection examples.
