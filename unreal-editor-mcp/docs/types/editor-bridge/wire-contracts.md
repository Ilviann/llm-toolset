...
The HTTP request has exactly two fields: `command` is one of the nine commands released through Phase 4; `arguments` is empty for `capabilities` and `editor_state`, follows the status contract for `operation_status`, follows one exact inspector shape for `blueprint_inspect`, or follows the owning mutation shape. Authentication is one `Authorization: Bearer <64 lowercase hex>` header. Requests with duplicate/missing authorization, an unknown command, fields, or invalid arguments are rejected before Blueprint access.
