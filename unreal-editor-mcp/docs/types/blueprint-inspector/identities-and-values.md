...
Each identity-bearing record contains `id` and `identity_stable`. If Unreal provides no valid GUID, `id` is empty and `identity_stable` is false; the inspector does not invent a mutable-object identity. Connection records reference graph, node, and pin IDs. Inherited component, variable, and graph records include their owning Blueprint and set `inherited`.
...
Variable and pin defaults are bounded strings, with a bounded object path for a pin default object. Component records encode only changed editable non-transient properties, up to 16 per component. Boolean, numeric, name/string/text, enum, common math/color/transform structs, and object/class references use `{name,supported:true,type,value}`. Other reflected types use `{name,supported:false,type:"unsupported"}`. `changed_default_count` and `defaults_truncated` make omitted values explicit; no arbitrary UObject graph is serialized.
