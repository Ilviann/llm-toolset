...
- Inspection: `mode: "inspect"`, one exact mounted-content `asset_path`, optional `sections`, optional exact 32-character `graph_id` or `component_id`, optional one-to-32 exact `property_names`, optional `include_inherited`, and optional `page_size`.
