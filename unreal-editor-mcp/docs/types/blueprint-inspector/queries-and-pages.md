...
Every success contains `mode`, a 40-character `snapshot_id`, `records`, `record_count`, `page_offset`, `scan_truncated`, and `has_more`. A partial page also contains `next_cursor` and `cursor_expires_in_ms`. Records carry a `section` discriminator including `variable`, `function`, `macro`, `custom_event`, `parameter`, `local_variable`, graph structure, defaults, and summary records. Editable declaration records include everything required to preflight the corresponding accepted `blueprint_member_edit` request.
