...
Blueprint Function Library assets report `function_library`; their function, parameter, local-variable, and graph records reuse the ordinary bounded forms. Blueprint Macro Library assets report `macro_library`; their macro, parameter, and graph records reuse the ordinary bounded forms. Library declaration records always report `editable: false`, and their live capabilities expose only the graph type owned by that library. Discovery uses the registry `BlueprintType` tag and exact inspection revalidates the loaded Blueprint type, so a Macro Library's scope parent never changes its family.
...
Member-variable defaults use the tagged canonical K2 forms documented with the mutator and are reconstructed from the generated-class CDO after compile. Pin records keep bounded raw `default_value`, optional `default_object`/`default_text`, and a tagged `default` reconstructed through the shared K2 codec; unavailable types remain explicit. An `FGameplayAttribute` default uses `{kind:"gameplay_attribute",resolved,compatible,name,property_path,owner_path}`; the same object is the `value` of a supported targeted reflected property whose `type` is `gameplay_attribute`. The parser accepts Unreal Engine 5.7's bounded `AttributeName`, `Attribute`, and `AttributeOwner` export fields, while the resolved field path remains authoritative. Resolution is live and compatibility accepts floating-point or `FGameplayAttributeData`-derived fields. This is inspection-only and does not add Gameplay Attribute mutation.

Component and `class_default` records encode only requested properties or bounded changed defaults. Boolean, finite numeric, name/string/text, enum/flags, gameplay tag/container, GUID, common math/color/transform structs, compatible hard/soft object/class references, arrays, deterministic maps, and bounded nested reflected structs use `{name,supported:true,type,value}`. Gameplay tags and GUIDs are strings, tag containers and arrays are arrays, maps use `{kind:"map",entries:[{key:…,value:…}]}` with at most 64 entries sorted by canonical encoded entry, references are visible packageable object/class paths, and general structs are `{kind:"struct",fields:{…}}`. An exact reflected `FGameplayModifierInfo` array uses this same form and admits the two additional internal levels needed for magnitude curves and tag queries without changing the published general depth limit. Other reflected types use `{name,supported:false,type:"unsupported"}`. Delegates, interfaces, transient/editor-only fields, sets, and arbitrary UObject graphs are never writable or recursively serialized; map mutation remains unsupported.
