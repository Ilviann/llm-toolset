...
The token is exactly 64 lowercase hexadecimal characters and is never included in results. `UnrealBridge.call` accepts the eleven Phase 10 commands, sends `{command,arguments}` to `/unreal-mcp/v1/command`, and requires `{ok:true,result}` or `{ok:false,error}`. Python schemas enforce exact discovery/inspection/continuation and all eight action-catalog families, operation reconciliation, creation/compile/save arguments, discriminated component operations, class-default edits, and variable/function/local/macro/custom-event shapes with bounded K2 signatures, types, defaults, metadata, identities, and event-graph targeting.
