...
The token is exactly 64 lowercase hexadecimal characters and is never included in results. `UnrealBridge.call` accepts the thirteen Phase 16 commands. Python schemas additionally enforce exact RPC metadata, typed Actor/component replication forms, and the fixed project-hash/current-class framework assignment shape.
...
Every model-facing error is `{code,message,details,retryable}`. Codes are defined by `ErrorCode`; messages are limited to 512 characters and details to 16 primitive fields. Stable `invalid_member`, `unsupported_type`, and `referenced_member` codes cover variable, callable, local, and RepNotify failures. Graph errors distinguish invalid/protected nodes and pins, invalid defaults, duplicate/missing/cyclic/incompatible connections, conversion requirements, no-change requests, and graph/default/link/conversion limits. Unknown native codes become `internal_error`.
