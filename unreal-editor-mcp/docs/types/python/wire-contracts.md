...
The token is exactly 64 lowercase hexadecimal characters and is never included in results. `UnrealBridge.call` accepts the seventeen commands released through `level-open` plus internal `editor_shutdown`. Python schemas additionally enforce exact level discovery/current/cursor shapes, exact mounted World paths for `level_open`, RPC metadata, typed Actor/component replication forms, fixed project-hash/current-class framework assignment, exact struct/table game-data operations with four-level recursive row values, and the large-mode lifecycle discriminator.
