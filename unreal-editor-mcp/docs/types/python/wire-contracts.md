...
The token is exactly 64 lowercase hexadecimal characters and is never included in results. `UnrealBridge.call` accepts the six Phase 3 commands, sends `{command,arguments}` to `/unreal-mcp/v1/command`, and requires `{ok:true,result}` or `{ok:false,error}`. Python tool schemas enforce the three exact inspection argument families and the exact creation/compile/save shapes documented under their owning type folders.
