...
The token is exactly 64 lowercase hexadecimal characters and is never included in results. `UnrealBridge.call` accepts only `capabilities` or `editor_state`, sends `{command,arguments}` to `/unreal-mcp/v1/command`, and requires `{ok:true,result}` or `{ok:false,error}`.
