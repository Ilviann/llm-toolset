...
The token is exactly 64 lowercase hexadecimal characters and is never included in results. `UnrealBridge.call` accepts the twenty-one model-facing commands released through `level-inspect` plus internal `editor_shutdown`. Python schemas additionally enforce exact asset-reference target/cursor shapes, exact stale-safe asset-delete arguments, exact level discovery/current/actor-list/actor/component/cursor shapes, map-qualified Actor GUIDs, bounded exact filters/properties, exact mounted World paths for `level_open`, RPC metadata, typed Actor/component replication forms, exact complete-function replacement plans, thirteen exact widget-tree operations, fixed project-hash/current-class framework assignment, exact struct/table game-data operations with four-level recursive row values, and the large-mode lifecycle discriminator.
