...
The token is exactly 64 lowercase hexadecimal characters and is never included in results. `UnrealBridge.call` accepts the ten Phase 5 commands, sends `{command,arguments}` to `/unreal-mcp/v1/command`, and requires `{ok:true,result}` or `{ok:false,error}`. Python schemas enforce exact discovery/inspection/continuation shapes, operation reconciliation, creation/compile/save arguments, discriminated component operations, class-default edits, and member add/rename/update/remove shapes with bounded K2 types/defaults/metadata.
...
Every model-facing error is `{code,message,details,retryable}`. Codes are defined by `ErrorCode`; messages are limited to 512 characters and details to 16 primitive fields. Phase 5 adds stable `invalid_member`, `unsupported_type`, and `referenced_member` codes to the Phase 4 mutation errors. Unknown native codes become `internal_error`.
