...
The token is exactly 64 lowercase hexadecimal characters and is never included in results. `UnrealBridge.call` accepts the nine Phase 4 commands, sends `{command,arguments}` to `/unreal-mcp/v1/command`, and requires `{ok:true,result}` or `{ok:false,error}`. Python schemas enforce exact discovery/inspection/continuation shapes, operation reconciliation, creation/compile/save arguments, discriminated component operations, and class-default edits.
