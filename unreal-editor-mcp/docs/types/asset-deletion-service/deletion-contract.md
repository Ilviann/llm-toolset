...
An ordinary target must be one persisted writable asset in one package under `/Game` or a symlink-free local project-plugin mount. A World target uses the map branch: the root map must not be current or loaded for streaming/sublevel use, and its bounded complete root/build-data/external actor/object package closure must be clean, writable, fully owned, and free of external registry or live-memory references. Direct external-package targeting remains forbidden.
