...
bool FUnrealMCPPhase4OperationLedgerTest::RunTest(const FString& Parameters)
{
    ...
    const TSharedRef<FUnrealMCPRecord> Committed = MakeShared<FUnrealMCPRecord>();
    ...
    const TSharedRef<FUnrealMCPRecord> PartialArguments = MakeShared<FUnrealMCPRecord>();
    ...
    const TSharedRef<FUnrealMCPRecord> Partial = MakeShared<FUnrealMCPRecord>();
    ...
    const TSharedRef<FUnrealMCPRecord> PartialStatusArguments = MakeShared<FUnrealMCPRecord>();
    ...
    TSharedPtr<FUnrealMCPRecord> PartialStatus;
    TestTrue(
    ...
    const TSharedRef<FUnrealMCPRecord> CommittedIdentity = MakeShared<FUnrealMCPRecord>();
    ...
    const TSharedRef<FUnrealMCPRecord> Queued = MakeShared<FUnrealMCPRecord>();
    Queued->SetStringField(TEXT("operation_id"), CancelId);
    Ledger.Admit(TEXT("blueprint_compile"), Queued);
    const TSharedRef<FUnrealMCPRecord> StatusArguments = MakeShared<FUnrealMCPRecord>();
    ...
    const TSharedRef<FUnrealMCPRecord> StatusWithCancel = MakeShared<FUnrealMCPRecord>(*StatusArguments);
    ...
    const TSharedRef<FUnrealMCPRecord> ExecutingRequest = MakeShared<FUnrealMCPRecord>();
    ...
    const TSharedRef<FUnrealMCPRecord> ExecutingIdentity = MakeShared<FUnrealMCPRecord>();
    ...
    const TSharedRef<FUnrealMCPRecord> WrongInstanceRequest = MakeShared<FUnrealMCPRecord>();
    ...
    TestEqual(TEXT("wrong-instance fixture is admitted"),
        ...
        EUnrealMCPOperationAdmission::Accepted);
    const TSharedRef<FUnrealMCPRecord> WrongInstanceIdentity = MakeShared<FUnrealMCPRecord>();
    ...
    const TSharedRef<FUnrealMCPRecord> CancelWithExtra = MakeShared<FUnrealMCPRecord>(*WrongInstanceIdentity);
    ...
    for (int32 Index = 0; Index < UnrealMCP::MaxRetainedOperations + 1; ++Index)
    {
        ...
        const TSharedRef<FUnrealMCPRecord> Request = MakeShared<FUnrealMCPRecord>();
        ...
        BoundedLedger.Commit(Id, MakeShared<FUnrealMCPRecord>());
    }
...
bool FUnrealMCPPhase4PropertyCodecTest::RunTest(const FString& Parameters)
{
    ...
    TestTrue(TEXT("Boolean form writes"), UnrealMCP::PropertyCodec::Set(Text, TEXT("bVisible"), MakeShared<FUnrealMCPValueBoolean>(false), Changed, Error));
    TestFalse(TEXT("Boolean form reads back exactly"), Changed->GetBoolField(TEXT("value")));
    TestTrue(TEXT("finite numeric form writes"), UnrealMCP::PropertyCodec::Set(Text, TEXT("WorldSize"), MakeShared<FUnrealMCPValueNumber>(42.5), Changed, Error));
    TestEqual(TEXT("finite numeric form reads back exactly"), Changed->GetNumberField(TEXT("value")), 42.5);
    TestTrue(TEXT("text form writes"), UnrealMCP::PropertyCodec::Set(Text, TEXT("Text"), MakeShared<FUnrealMCPValueString>(TEXT("Phase Four")), Changed, Error));
    TestEqual(TEXT("text form reads back exactly"), Changed->GetStringField(TEXT("value")), FString(TEXT("Phase Four")));
    TestTrue(TEXT("enum form writes"), UnrealMCP::PropertyCodec::Set(Text, TEXT("HorizontalAlignment"), MakeShared<FUnrealMCPValueString>(TEXT("EHTA_Center")), Changed, Error));
    ...
    TestTrue(TEXT("safe struct form writes"), UnrealMCP::PropertyCodec::Set(Text, TEXT("TextRenderColor"),
        MakeShared<FUnrealMCPValueString>(TEXT("(R=10,G=20,B=30,A=255)")), Changed, Error));
    ...
    TestTrue(TEXT("string form writes"), UnrealMCP::PropertyCodec::Set(Capture, TEXT("ProfilingEventName"),
        MakeShared<FUnrealMCPValueString>(TEXT("UnrealMCP")), Changed, Error));
    ...
    TestTrue(TEXT("name form writes"), UnrealMCP::PropertyCodec::Set(Capture, TEXT("CollectionTransformWorldToLocal"),
        MakeShared<FUnrealMCPValueString>(TEXT("WorldToLocal")), Changed, Error));
    ...
    const TArray<TSharedPtr<FUnrealMCPValue>> Flags = {
        MakeShared<FUnrealMCPValueString>(TEXT("HLOD0")), MakeShared<FUnrealMCPValueString>(TEXT("HLOD2"))};
    const bool bFlagsWritten = UnrealMCP::PropertyCodec::Set(Mesh, TEXT("ExcludeFromHLODLevels"),
        MakeShared<FUnrealMCPValueArray>(Flags), Changed, Error);
    ...
    TestTrue(TEXT("visible engine asset reference writes"), UnrealMCP::PropertyCodec::Set(Mesh, TEXT("StaticMesh"),
        MakeShared<FUnrealMCPValueString>(TEXT("/Engine/BasicShapes/Cube.Cube")), Changed, Error));
    ...
    TestTrue(TEXT("soft asset reference writes"), UnrealMCP::PropertyCodec::Set(Cloud, TEXT("Material"),
        MakeShared<FUnrealMCPValueString>(TEXT("/Engine/EngineMaterials/DefaultMaterial.DefaultMaterial")), Changed, Error));
    ...
    TestTrue(TEXT("Blueprint class reference writes"), UnrealMCP::PropertyCodec::Set(Child, TEXT("ChildActorClass"),
        MakeShared<FUnrealMCPValueString>(ReferenceBlueprint->GeneratedClass->GetPathName()), Changed, Error));
    ...
    const bool bSoftClassWritten = UnrealMCP::PropertyCodec::Set(Input, TEXT("DefaultInputComponentClass"),
        MakeShared<FUnrealMCPValueString>(TEXT("/Script/Engine.InputComponent")), Changed, Error);
    ...
    TestFalse(TEXT("unsupported container rejects"), UnrealMCP::PropertyCodec::Set(Text, TEXT("ComponentTags"),
        MakeShared<FUnrealMCPValueArray>(TArray<TSharedPtr<FUnrealMCPValue>>()), Changed, Error));
...
bool FUnrealMCPPhase4ComponentAndDefaultTest::RunTest(const FString& Parameters)
{
    ...
    TSharedRef<FUnrealMCPRecord> AddRoot = ComponentEditArguments(AssetPath, Snapshot, TEXT("add"));
    ...
    TSharedRef<FUnrealMCPRecord> SetRoot = ComponentEditArguments(AssetPath, Snapshot, TEXT("set_root"));
    ...
    Snapshot = Result->GetStringField(TEXT("snapshot_id"));
    TSharedRef<FUnrealMCPRecord> AddMesh = ComponentEditArguments(AssetPath, Snapshot, TEXT("add"));
    ...
    Snapshot = Result->GetStringField(TEXT("snapshot_id"));
    TSharedRef<FUnrealMCPRecord> AddMovement = ComponentEditArguments(AssetPath, Snapshot, TEXT("add"));
    ...
    Snapshot = Result->GetStringField(TEXT("snapshot_id"));
    TSharedRef<FUnrealMCPRecord> Rename = ComponentEditArguments(AssetPath, Snapshot, TEXT("rename"));
    ...
    Snapshot = Result->GetStringField(TEXT("snapshot_id"));
    TSharedRef<FUnrealMCPRecord> AddPivot = ComponentEditArguments(AssetPath, Snapshot, TEXT("add"));
    ...
    Snapshot = Result->GetStringField(TEXT("snapshot_id"));
    TSharedRef<FUnrealMCPRecord> Reparent = ComponentEditArguments(AssetPath, Snapshot, TEXT("reparent"));
    ...
    Snapshot = Result->GetStringField(TEXT("snapshot_id"));
    TSharedRef<FUnrealMCPRecord> Cycle = ComponentEditArguments(AssetPath, Snapshot, TEXT("reparent"));
    ...
    TSharedRef<FUnrealMCPRecord> Duplicate = ComponentEditArguments(AssetPath, Snapshot, TEXT("add"));
    ...
    TSharedRef<FUnrealMCPRecord> InvalidClass = ComponentEditArguments(AssetPath, Snapshot, TEXT("add"));
    ...
    Snapshot = Result->GetStringField(TEXT("snapshot_id"));
    TSharedRef<FUnrealMCPRecord> SetVisible = ComponentEditArguments(AssetPath, Snapshot, TEXT("set_property"));
    ...
    const TSharedRef<FUnrealMCPRecord> SetActorDefault = MakeShared<FUnrealMCPRecord>();
    ...
    Snapshot = Result->GetStringField(TEXT("snapshot_id"));
    const TSharedRef<FUnrealMCPRecord> TargetedInspect = InspectArguments(AssetPath);
    TargetedInspect->SetArrayField(TEXT("sections"), {MakeShared<FUnrealMCPValueString>(TEXT("components")), MakeShared<FUnrealMCPValueString>(TEXT("class_defaults"))});
    TargetedInspect->SetStringField(TEXT("component_id"), MeshId);
    TargetedInspect->SetArrayField(TEXT("property_names"), {MakeShared<FUnrealMCPValueString>(TEXT("bVisible")), MakeShared<FUnrealMCPValueString>(TEXT("InitialLifeSpan"))});
    ...
    TSharedRef<FUnrealMCPRecord> Stale = ComponentEditArguments(AssetPath, FString::ChrN(40, TEXT('0')), TEXT("remove"));
    ...
    TSharedRef<FUnrealMCPRecord> RemoveVisual = ComponentEditArguments(AssetPath, Snapshot, TEXT("remove"));
