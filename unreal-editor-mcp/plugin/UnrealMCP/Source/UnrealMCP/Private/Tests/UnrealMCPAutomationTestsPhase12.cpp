...
bool FUnrealMCPPhase12PinDefaultsAndConnectionsTest::RunTest(const FString& Parameters)
{
    ...
    auto SetDefault = [&](UEdGraphNode* Node, UEdGraphPin* Pin, const TSharedRef<FUnrealMCPRecord>& Default) -> bool
    {
        TSharedRef<FUnrealMCPRecord> Arguments = Edit(TEXT("set_pin_default"));
    ...
    {
        TSharedRef<FUnrealMCPRecord> Arguments = Edit(Operation);
    ...
    TestTrue(TEXT("string pin default is schema parsed and set"), SetDefault(PrintOne, TextPin,
        LiteralDefault(MakeShared<FUnrealMCPValueString>(TEXT("Phase 12 direct behavior")))));
    TestEqual(TEXT("string pin retains canonical default"), TextPin->DefaultValue, FString(TEXT("Phase 12 direct behavior")));
    const TSharedRef<FUnrealMCPRecord> EngineDefault = MakeShared<FUnrealMCPRecord>();
    ...
    TestTrue(TEXT("Boolean pin default is schema parsed and set"), SetDefault(PrintOne, BoolPin,
        LiteralDefault(MakeShared<FUnrealMCPValueBoolean>(false))));
    ...
    TSharedRef<FUnrealMCPRecord> ProtectedDefault = Edit(TEXT("set_pin_default"));
    ...
    ProtectedDefault->SetObjectField(TEXT("default"), LiteralDefault(MakeShared<FUnrealMCPValueNumber>(1)));
    ...
    TSharedRef<FUnrealMCPRecord> MissingPin = Edit(TEXT("set_pin_default"));
    ...
    MissingPin->SetObjectField(TEXT("default"), LiteralDefault(MakeShared<FUnrealMCPValueString>(TEXT("stale"))));
