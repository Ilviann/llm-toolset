...
bool FUnrealMCPPhase15GameInstanceFamilyTest::RunTest(const FString& Parameters)
{
    ...
    TSharedPtr<FUnrealMCPRecord> GameInstanceRecord;
    ...
    const TSharedPtr<FUnrealMCPRecord> Operations = GameInstanceRecord->GetObjectField(TEXT("operations"));
    ...
    Inspect->SetArrayField(TEXT("sections"), {
        MakeShared<FUnrealMCPValueString>(TEXT("summary")), MakeShared<FUnrealMCPValueString>(TEXT("components")),
    ...
    const TSharedPtr<FUnrealMCPRecord> Live = Result->GetObjectField(TEXT("family_capabilities"));
    ...
    const TSharedPtr<FUnrealMCPRecord> GraphTypes = Live->GetObjectField(TEXT("graph_types"));
    ...
    const TArray<TSharedPtr<FUnrealMCPValue>>& InitialRecords = Result->GetArrayField(TEXT("records"));
    bool bActorSummary = true;
    int32 ComponentRecords = 0;
    for (const TSharedPtr<FUnrealMCPValue>& Value : InitialRecords)
    {
        const TSharedPtr<FUnrealMCPRecord> Record = Value->AsObject();
    ...
    TSharedRef<FUnrealMCPRecord> Component = ComponentEditArguments(AssetPath, InitialSnapshot, TEXT("add"));
    ...
    TSharedRef<FUnrealMCPRecord> AddState = MemberEditArguments(AssetPath, InitialSnapshot, TEXT("add"));
    AddState->SetStringField(TEXT("name"), TEXT("SessionRegion"));
    AddState->SetObjectField(TEXT("type"), K2Type(TEXT("string")));
    AddState->SetObjectField(TEXT("default"), LiteralDefault(MakeShared<FUnrealMCPValueString>(TEXT("offline"))));
    ...
    TSharedRef<FUnrealMCPRecord> CompileMember = AssetArguments(AssetPath);
    ...
    TSharedRef<FUnrealMCPRecord> Default = MakeShared<FUnrealMCPRecord>();
    ...
    TSharedRef<FUnrealMCPRecord> AddFunction = ScopedMemberEditArguments(
    ...
    TSharedRef<FUnrealMCPRecord> AddLocal = ScopedMemberEditArguments(
    ...
    TestEqual(TEXT("GameInstance callback catalog retains family"),
        Result->GetStringField(TEXT("blueprint_family")), FString(TEXT("game_instance")));
    const TArray<TSharedPtr<FUnrealMCPValue>>& Actions = Result->GetArrayField(TEXT("actions"));
    ...
    const TSharedRef<FUnrealMCPRecord> Position = MakeShared<FUnrealMCPRecord>();
    Position->SetNumberField(TEXT("x"), 160);
    Position->SetNumberField(TEXT("y"), 120);
    const TSharedRef<FUnrealMCPRecord> AddCallback = MakeShared<FUnrealMCPRecord>();
    ...
    TSharedRef<FUnrealMCPRecord> ReadBack = InspectArguments(AssetPath);
