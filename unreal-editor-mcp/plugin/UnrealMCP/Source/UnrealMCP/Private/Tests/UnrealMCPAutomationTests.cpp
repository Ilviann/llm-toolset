...
bool FUnrealMCPPhase5MemberVariableTest::RunTest(const FString& Parameters)
{
    ...
    TSharedRef<FJsonObject> RenameRepNotify = MemberEditArguments(AssetPath, Snapshot, TEXT("rename"));
    RenameRepNotify->SetStringField(TEXT("member_id"), HealthId);
    RenameRepNotify->SetStringField(TEXT("new_name"), TEXT("HealthAgain"));
    TestFalse(TEXT("RepNotify rename is deferred"), Mutator.Execute(TEXT("blueprint_member_edit"), RenameRepNotify, Result, Error));
    TestEqual(TEXT("RepNotify rejection preserves snapshot"), InspectSnapshot(Inspector, AssetPath), Snapshot);
