    ...
    TArray<FNamedSlotRef>& OutSlots)
{
    ...
    auto AddHost = [&OutSlots](UObject* Host, const FString& HostId, const bool bTreeHost)
    {
        ...
        {
            OutSlots.Add({Host, NamedSlotId(HostId, Name), HostId, Name,
                Interface->GetContentForSlot(Name), bTreeHost});
        }
    };
...
inline int32 ComputeDepth(UWidgetBlueprint* Blueprint, const TArray<UWidget*>& Widgets,
    const TArray<FNamedSlotRef>& NamedSlots, bool& bOutCycle)
{
    bOutCycle = false;
    if (Blueprint == nullptr || Blueprint->WidgetTree == nullptr)
    {
