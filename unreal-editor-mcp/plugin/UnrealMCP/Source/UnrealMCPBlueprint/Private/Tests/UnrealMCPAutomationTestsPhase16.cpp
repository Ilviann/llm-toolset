...
namespace
{
TSharedRef<FUnrealMCPRecord> EmptyEventSignature()
{
...
TSharedRef<FUnrealMCPRecord> FrameworkArguments(
...
bool FUnrealMCPPhase16MultiplayerAuthoringTest::RunTest(const FString& Parameters)
{
    ...
    const TArray<TSharedPtr<FUnrealMCPValue>> Matrix = BuildPublishedMatrix();
    TestEqual(TEXT("family matrix remains bounded"), Matrix.Num(), 7);
    for (const TSharedPtr<FUnrealMCPValue>& Value : Matrix)
    {
        const TSharedPtr<FUnrealMCPRecord> Record = Value->AsObject();
        if (!Record.IsValid()) continue;
        const FString Family = Record->GetStringField(TEXT("family"));
        const TSharedPtr<FUnrealMCPRecord> Operations = Record->GetObjectField(TEXT("operations"));
        ...
        TestTrue(*FString::Printf(TEXT("%s publishes multiplayer detail"), *Family),
            Record->HasTypedField<EUnrealMCPValueType::Record>(TEXT("multiplayer")));
    }
    ...
    auto SetReplication = [&](const TCHAR* Setting, const TSharedPtr<FUnrealMCPValue>& Value)
    {
        const TSharedRef<FUnrealMCPRecord> Arguments = MakeShared<FUnrealMCPRecord>();
        Arguments->SetStringField(TEXT("operation_id"), FGuid::NewGuid().ToString(EGuidFormats::Digits).ToLower());
    ...
    TestTrue(TEXT("Actor replication enables"), SetReplication(TEXT("replicates"), MakeShared<FUnrealMCPValueBoolean>(true)));
    TestTrue(TEXT("movement replication enables after Actor replication"),
        SetReplication(TEXT("replicate_movement"), MakeShared<FUnrealMCPValueBoolean>(true)));
    TestTrue(TEXT("net priority sets through typed path"),
        SetReplication(TEXT("net_priority"), MakeShared<FUnrealMCPValueNumber>(2.5)));

    TSharedRef<FUnrealMCPRecord> AddComponent = ComponentEditArguments(AssetPath, Snapshot, TEXT("add"));
    ...
    TSharedRef<FUnrealMCPRecord> SetComponent = ComponentEditArguments(AssetPath, Snapshot, TEXT("set_replication"));
    ...
    {
        TSharedRef<FUnrealMCPRecord> Arguments = ScopedMemberEditArguments(AssetPath, Snapshot, TEXT("custom_event"), TEXT("add"));
    ...
    TSharedRef<FUnrealMCPRecord> Invalid = ScopedMemberEditArguments(AssetPath, Snapshot, TEXT("custom_event"), TEXT("add"));
    ...
    const TSharedRef<FUnrealMCPRecord> InvalidMetadata = MakeShared<FUnrealMCPRecord>();
    ...
    TSharedRef<FUnrealMCPRecord> Inspect = InspectArguments(AssetPath);
    Inspect->SetArrayField(TEXT("sections"), {MakeShared<FUnrealMCPValueString>(TEXT("components")),
        MakeShared<FUnrealMCPValueString>(TEXT("class_defaults")), MakeShared<FUnrealMCPValueString>(TEXT("custom_events"))});
    Inspect->SetArrayField(TEXT("property_names"), {MakeShared<FUnrealMCPValueString>(TEXT("bReplicates")),
        MakeShared<FUnrealMCPValueString>(TEXT("bReplicateMovement")), MakeShared<FUnrealMCPValueString>(TEXT("NetPriority"))});
    ...
    TestTrue(TEXT("live multiplayer capabilities are published"),
        Result->GetObjectField(TEXT("family_capabilities"))->HasTypedField<EUnrealMCPValueType::Record>(TEXT("multiplayer")));
